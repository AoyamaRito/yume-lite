...
yume-constraint-voxel（本格的な3Dボクセル実装 + Snow-Ball 100% coverage 実践例）から**エッセンスだけを抽出**して作りました。
...
voxel は「実際に大きな状態を constraint で回した例」として非常に価値が高いです。  
この simple は「その前にこれを読め」という入門編の位置づけです。
...
4. `yume-constraint-voxel/`（本格実践例 + Snow-Ball 方法論、外部リファレンス）
5. 他の yume 系プロジェクトでこのパターンがどう使われているかを見る
